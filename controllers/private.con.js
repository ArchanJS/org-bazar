...
exports.getItems=async(req,res)=>{
    try {
        const items=await Item.find({sold:false,isApproved:true}).populate('owner').populate("tags");
  ...
  exports.informUser=async(req,res)=>{
    try {
        ...
        sendEmail({
            user:user.email,
            subject:"New chat",
            html:`<h1>New chat is waiting for you!!!</h1><br><h4>Hi, ${user.username}. Click on the following button to join chat.</h4><br><a href="https://org-bazar.herokuapp.com/chat/${roomId}/${otherId}/${userId}" target="_blank"><button style="color: white;background: purple;cursor: pointer;">Click here to join chat</button></a>`
        })
    } catch (error) {
        console.log(error);
        res.status(500).json({ error: "Something went wrong!" });
...
exports.removeItem=async(req,res)=>{
    try {
        await User.update({wishlist:req.params.itemId},{
